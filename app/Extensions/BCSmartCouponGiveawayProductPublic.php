...
class BCSmartCouponGiveawayProductPublic extends Wt_Smart_Coupon_Giveaway_Product
{
	...
	public function add_giveaway_products_with_coupon( $valid, $coupon ) {
		...
		if ( self::is_bogo( $coupon ) ) {
			...
			} elseif ( 'any_product_from_category' === $bogo_customer_gets || 'any_product_from_store' === $bogo_customer_gets || 'any_product_from_category_in_the_cart' === $bogo_customer_gets ) {
				/* any product from store or any product from specific category */
	...
	private function specific_product_add_to_cart( $coupon, $coupon_id, $coupon_code, $bogo_customer_gets, $product_id, $variation_id ) {
		...
		if ( 'and' == $bogo_product_condition ) {
			...
			if ( ! empty( $bogo_products ) ) {
				/* after checking the existing items, any remaining items to be added */
	 ...
	 * If variable product, checks any of the variation is purchasable, and returns the variation id if successful, otherwise false will return
	 *
	 * @since 2.0.4
			...
			if ( empty( $variations ) && false !== $variations ) {
				...
				foreach ( $variations as $variation ) {
					...
					if ( $this->is_purchasable( $variation_product ) ) {
						/* any of the product is purchasable */
				...
				if ( ! $is_purchasable ) {
					/* all variations are not purchasable */
					return false;
				} else {
					...
					if ( ! empty( $coupon_code ) ) {
						...
						if ( $coupon && ! self::is_bogo( $coupon ) ) {
							...
							if ( wc_string_to_bool( $this->get_coupon_meta_value( $coupon_id, 'wt_apply_discount_before_tax_calculation' ) ) === true ) {
								/* currently only applicable for non BOGO */
								continue;
							}
	...
	public function check_any_free_products_without_coupon() {
		...
		if ( is_object( $cart ) && is_callable( array( $cart, 'is_empty' ) ) && ! $cart->is_empty() ) {
			...
			foreach ( $cart_items as $cart_item_key => $cart_item ) {
				if ( self::is_a_free_item( $cart_item ) ) {
					if ( ! in_array( $cart_item['free_gift_coupon'], $coupons ) ) {
						/* coupon not found in the applied coupon list */
	...
	private function apply_cheapest_giveaway_for_any_product_from_category( $coupon, $coupon_id, $coupon_code ) {
		...
		foreach ( $bogo_free_category_ids as $category_id ) {
			...
			$cart_items = WC()->cart->get_cart(); // take a fresh list every time.
			...
			if ( ! empty( $temp_arr ) ) { // items present in the current category
				...
				// this is used to run multiple iterations when required
	 ...
	 * @since 2.0.7
	 * @param string $cart_item_key Cart item key
	 * @param int    $quantity Quantity to be converted as giveaway. Optional argument. If no quantity given then the whole cart item quantity is converted as normal cart item
	 */
	 ...
	 *
	 * @since 2.0.7
	 * @param int    $to_remove_qty  How many quantity to be removed (Reference argument)
	 * @param array  $cart_item      Cart item
	 * @param string $cart_item_key  Cart key
	 * @param int    $cart_item_qty  The quantity available for removal, Optional, If not specified then remove all the cart item quantity
	 */
