...
class Woocommerce {
	...
	public function __construct() {
		...
		add_action( 'bc_schedule_product_update_sync', array( $this, 'process_product_update_sync' ) );
	}
	...
	public function schedule_product_sync( $product_id ) {
		...
		as_unschedule_action( 'bc_schedule_product_update_sync', array( 'product_id' => $product_id ), 'blaze-commerce' );
		as_schedule_single_action( $event_time, 'bc_schedule_product_update_sync', array( 'product_id' => $product_id ), 'blaze-commerce', true, 1 );
	}

	public function process_product_update_sync( $product_id ) {
		$product = wc_get_product( $product_id );
		if ( $product ) {
			$this->on_product_save( $product->get_id(), $product );
		}
	}
	...
	public function is_rest_api_request() {
		...
		// Alternative check for REST API URL patterns
		if ( ! empty( $_SERVER['REQUEST_URI'] ) && strpos( $_SERVER['REQUEST_URI'], '/wp-json/wc/v' ) !== false ) {
			return true;
		}
	...
	public function on_product_save( $product_id, $wc_product ) {
		...
		try {
			...
			$logger        = wc_get_logger();
			$context       = array( 'source' => 'taxonomies-data' );
			$logger->debug( print_r(
				array(
					'product_id' => $product_id,
					'is_rest_api_request' => $this->is_rest_api_request(),
					'taxonomies' => $document_data['taxonomies']
				), true
			), $context );
