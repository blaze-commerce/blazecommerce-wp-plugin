...
class Product extends BaseCollection {
	...
	public function get_all_child_categories_recursive( $term_id, $taxonomy, &$child_terms = [] ) {
		$children = get_term_children( $term_id, $taxonomy );
		if ( ! empty( $children ) && ! is_wp_error( $children ) ) {
			foreach ( $children as $child_id ) {
				if ( ! in_array( $child_id, $child_terms ) ) {
					$child_terms[] = $child_id;
					// Recursively get the children of this child
					$this->get_all_child_categories_recursive( $child_id, $taxonomy, $child_terms );
				}
			}
		}
		return $child_terms;
	...
	public function get_all_categories_with_parents( $product_id ) {
		...
		foreach ( $current_categories as $category_id ) {
			...
			// $child_categories = $this->get_all_child_categories_recursive( $category_id, 'product_cat' );
			// foreach ( $child_categories as $child_category_id ) {
			// 	if ( ! in_array( $child_category_id, $all_categories ) ) {
			// 		$all_categories[] = $child_category_id;
			// 	}
			// }
		}
